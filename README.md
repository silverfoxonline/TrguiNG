...
### 更新 (260716b)
1. add: 表格标题增加多级排序菜单，每列可选择升序、降序或默认，并按选择先后显示排序优先级。
2. impr: 多列排序条件会综合生效并保存，选择默认只移除当前列的排序条件。
3. build: GitHub Actions 仅在 Ubuntu 上构建 WebUI ZIP；推送版本标签时自动附加到草稿 Release。
4. add: 工具栏增加恢复默认排序按钮，可一键取消种子列表的全部排序条件。
5. impr: 同一种子的多个用户标签按英文在前、中文在后的顺序进行忽略大小写和数字自然排序。
6. add: 用户标签筛选支持“包含/排除”三态选择，并可在“任一”和“全部”模式间切换，实现标签的或、与及排除组合筛选。
